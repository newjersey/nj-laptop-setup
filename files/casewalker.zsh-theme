...
function custom_git_prompt() {
  ...
  if [ "${ref}" = "$(git rev-parse --short HEAD 2> /dev/null)" ] ; then
    echo "$ZSH_THEME_GIT_PROMPT_PREFIX%{$R%}${ref#refs/heads/}%{$B%}$(parse_git_dirty)$(git_prompt_ahead)$(custom_git_prompt_status)$ZSH_THEME_GIT_PROMPT_SUFFIX"
  else
...
PROMPT='%B$PROMPTPREFIX %{$fg_bold[green]%}%~%{$RESET%} $(custom_git_prompt)%{$M%}%B$%b%{$RESET%} '
RPS1="${return_code}"
